        ...
        <Page title="Benchmarking"
            ...
            <Block title="Norm Creation Wizard"
                ...
                <template #actions>
                    ...
                    <UButton label="Create Dataset" icon="i-lucide-pen-line" size="lg" to="norm-creation/create" />
                </template>

                <!-- component -->
                <UCard v-for="(dataset, index) in [
                    { title: 'Endurance Test', description: 'Benchmark for long-distance running performance', inputs: ['VO2 Max', 'Heart Rate', 'Stamina'] },
                    { title: 'Strength Assessment', description: 'Benchmark for overall muscular strength', inputs: ['Bench Press', 'Squat', 'Deadlift'] },
                    { title: 'Flexibility Measure', description: 'Benchmark for joint flexibility and range of motion', inputs: ['Sit and Reach', 'Shoulder Flexibility'] },
                    { title: 'Agility Drill', description: 'Benchmark for quickness and change of direction', inputs: ['T-Test', 'Pro Agility Shuttle'] },
                    { title: 'Speed Test', description: 'Benchmark for sprinting speed', inputs: ['40-Yard Dash', '100-Meter Sprint'] }
...
import { ref } from 'vue'
const selected = ref<boolean[]>(Array(5).fill(false))
const page = ref(1)
</script>